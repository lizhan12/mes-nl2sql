...
import {
  createEvolvedFewShot,
  createFewShot,
  deleteEvolvedFewShot,
  deleteFewShot,
  fetchEvolvedFewShots,
  fetchFewShots,
  toggleEvolvedFewShot,
  toggleFewShot,
  updateEvolvedFewShot,
  updateFewShot,
} from "@/lib/api";
import type { DedupSimilarItem, EvolvedFewShotItem, FewShotItem } from "@/types";
...
function FewShotEditModal({
  ...
  onSave: (scenario: string, question: string, sql: string) => Promise<void>;
  onClose: () => void;
}) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await onSave(scenario.trim(), question.trim(), sql.trim());
      onClose();
    } catch (e) {
      alert(`保存失败: ${e}`);
    } finally {
...
function EvolvedFewShotEditModal({
  item,
  onSave,
  onClose,
}: {
  item: EvolvedFewShotItem | null;
  onSave: (scenario: string, question: string, sql: string) => Promise<void>;
  onClose: () => void;
}) {
  const [scenario, setScenario] = useState(item?.scenario || "");
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="w-[600px] rounded-lg border border-[var(--border-default)] bg-white p-5 shadow-xl">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-[var(--text-primary)]">
            {item ? "编辑 EvolvedFewShot" : "新增 EvolvedFewShot"}
          </h3>
          <button
            onClick={onClose}
            className="rounded p-1 text-[var(--text-tertiary)] hover:bg-[var(--bg-hover)]"
        ...
        <div className="space-y-3">
          ...
          <label className="block space-y-1">
            ...
            <input
              ...
              value={question}
              onChange={(e) => setQuestion(e.target.value)}
              placeholder="例如：查询工单 WO20240001 的过站记录"
              className="w-full rounded border border-[var(--border-default)] bg-[var(--bg-default)] px-2.5 py-1.5 text-xs text-[var(--text-primary)] focus:border-[var(--accent)] focus:outline-none"
            />
          </label>
          <label className="block space-y-1">
...
export default function FewShotManagement() {
  ...
  // EvolvedFewShot 状态
  const [evolvedFewShots, setEvolvedFewShots] = useState<EvolvedFewShotItem[]>([]);
  const [loadingEvolvedFewShots, setLoadingEvolvedFewShots] = useState(true);
  const [evolvedFewShotEditModal, setEvolvedFewShotEditModal] = useState<{
    open: boolean;
    item: EvolvedFewShotItem | null;
  }>({ open: false, item: null });
  ...
  // 加载 EvolvedFewShots
  const loadEvolvedFewShots = useCallback(async () => {
    setLoadingEvolvedFewShots(true);
    try {
      const data = await fetchEvolvedFewShots();
      setEvolvedFewShots(data);
    } catch (e) {
      console.error("加载 EvolvedFewShot 失败:", e);
    } finally {
      setLoadingEvolvedFewShots(false);
    }
  }, []);
  ...
  useEffect(() => {
    loadFewShots();
    loadEvolvedFewShots();
  }, [loadFewShots, loadEvolvedFewShots]);

  // FewShot 操作
  const handleCreateFewShot = async (scenario: string, question: string, sql: string) => {
    await handleCreateWithDedup(
      (force) => createFewShot(scenario, question, sql, force),
      loadFewShots,
    );
  };

  const handleUpdateFewShot = async (scenario: string, question: string, sql: string) => {
    if (!fewShotEditModal.item) return;
    await updateFewShot(fewShotEditModal.item.id, scenario, question, sql);
    await loadFewShots();
  };
  ...
  // EvolvedFewShot 操作
  const handleCreateEvolvedFewShot = async (scenario: string, question: string, sql: string) => {
    await handleCreateWithDedup(
      (force) => createEvolvedFewShot(scenario, question, sql, force),
      loadEvolvedFewShots,
    );
  };

  const handleUpdateEvolvedFewShot = async (scenario: string, question: string, sql: string) => {
    if (!evolvedFewShotEditModal.item) return;
    await updateEvolvedFewShot(evolvedFewShotEditModal.item.id, scenario, question, sql);
    await loadEvolvedFewShots();
  };

  const handleDeleteEvolvedFewShot = async (id: string) => {
    if (!confirm("确定删除此 EvolvedFewShot？")) return;
    try {
      await deleteEvolvedFewShot(id);
      await loadEvolvedFewShots();
    } catch (e) {
      alert(`删除失败: ${e}`);
    }
  ...
  const handleToggleEvolvedFewShot = async (id: string, enabled: boolean) => {
    try {
      await toggleEvolvedFewShot(id, enabled);
      await loadEvolvedFewShots();
    } catch (e) {
      alert(`操作失败: ${e}`);
    }
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        {/* EvolvedFewShot 管理 */}
        <Panel
          title={`EvolvedFewShot 示例 (${evolvedFewShots.length})`}
          action={
            <button
              onClick={() => setEvolvedFewShotEditModal({ open: true, item: null })}
              className="inline-flex items-center gap-1 rounded bg-[var(--accent)] px-3 py-1.5 text-xs font-medium text-white hover:opacity-90"
            >
              <Plus size={13} />
              新增 EvolvedFewShot
        ...
        >
          {loadingEvolvedFewShots ? (
            <div className="flex items-center justify-center py-12">
              <Loader2 size={20} className="animate-spin text-[var(--text-tertiary)]" />
            </div>
          ) : evolvedFewShots.length === 0 ? (
            <div className="py-12 text-center text-sm text-[var(--text-tertiary)]">
              暂无 EvolvedFewShot 示例
            </div>
          ) : (
            <div className="space-y-3">
              {evolvedFewShots.map((item) => (
                <div
                  key={item.id}
                  className="rounded border border-[var(--border-default)] bg-[var(--bg-raised)] p-3"
                >
                    ...
                    <div className="flex items-center gap-1">
                      <label className="relative inline-flex cursor-pointer items-center" title={item.enabled !== false ? "禁用" : "启用"}>
                        <input
                          ...
                          className="peer sr-only"
                          checked={item.enabled !== false}
                          onChange={() => handleToggleEvolvedFewShot(item.id, item.enabled === false)}
                        />
                        <span className="h-5 w-9 rounded-full border border-[var(--border-default)] bg-[var(--bg-subtle)] transition-colors peer-checked:bg-[var(--accent)] peer-checked:border-[var(--accent)] after:absolute after:left-[2px] after:top-[2px] after:h-4 after:w-4 after:rounded-full after:bg-white after:shadow-sm after:transition-transform peer-checked:after:translate-x-[16px]"></span>
                      </label>
                      <button
                        onClick={() => setEvolvedFewShotEditModal({ open: true, item })}
                        className="rounded p-1 text-[var(--text-tertiary)] hover:bg-[var(--bg-hover)] hover:text-[var(--accent)]"
                        title="编辑"
                      >
                        <Edit3 size={14} />
                      ...
                      <button
                        onClick={() => handleDeleteEvolvedFewShot(item.id)}
                        className="rounded p-1 text-[var(--text-tertiary)] hover:bg-[var(--bg-hover)] hover:text-[var(--error)]"
                        title="删除"
                      >
                        <Trash2 size={14} />
      ...
      {/* EvolvedFewShot 编辑弹窗 */}
      {evolvedFewShotEditModal.open && (
        <EvolvedFewShotEditModal
          item={evolvedFewShotEditModal.item}
          onSave={
            evolvedFewShotEditModal.item
              ? handleUpdateEvolvedFewShot
              : handleCreateEvolvedFewShot
          }
          onClose={() => setEvolvedFewShotEditModal({ open: false, item: null })}
        />
      )}
