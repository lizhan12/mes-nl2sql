                ...
                {isExpanded && (
                  <div className="mt-1 ml-4 pl-4 border-l-2 border-blue-200 space-y-1 py-1">
                    {span.input_json && Object.keys(span.input_json).length > 0 && (
                      <CodeBlock title="输入" value={span.input_json} language="json" maxHeightClassName="max-h-48" />
                    )}
                    {span.output_json && Object.keys(span.output_json).length > 0 && (
                      <CodeBlock title="输出" value={span.output_json} language="json" maxHeightClassName="max-h-48" />
                    )}
                      ...
                      <div key={llm.span_id} className="bg-purple-50 rounded px-2 py-1">
                        ...
                        {showLlmDetail === llm.span_id && (
                          <div className="mt-1 text-xs text-gray-600 space-y-1">
                            ...
                            {llm.prompt_preview && (
                              <div className="bg-white rounded p-1 text-xs font-mono text-gray-500 max-h-60 overflow-y-auto whitespace-pre-wrap">
                                {llm.prompt_preview}
                              </div>
                            )}
