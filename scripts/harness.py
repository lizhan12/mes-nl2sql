...
def run_harness(base_url: str, report_path: Path, max_workers: int, evolve: bool) -> None:
    ...
    summary = {
        "total": len(report),
        "generated_exec_ok": sum(1 for item in report if (item.get("generated_execution_result") or {}).get("success")),
...
def auto_label_failures_online(
    limit: int = 50,
    ...
    """LLM 自动标注失败案例 + 多维度评估。"""
    db_url = settings.execution_database_url
    summary = auto_label_failures_online_service(
        limit=limit,
...
def review_candidate(candidate_id: int, action: str, note: str = "") -> None:
    print(
        json.dumps(
            review_candidate_service(candidate_id=candidate_id, action=action, note=note), ensure_ascii=False, indent=2
        )
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    auto_label_parser = subparsers.add_parser("auto-label-failures", help="LLM 自动标注 + 多维度评估失败案例")
    auto_label_parser.add_argument("--limit", type=int, default=50, help="最多处理多少条失败案例")
    auto_label_parser.add_argument("--skip-sync-failures", action="store_true", help="跳过失败案例同步")
    auto_label_parser.add_argument("--generate-model", default="", help="生成 SQL 用的 LLM 模型")
    auto_label_parser.add_argument("--eval-model", default="", help="评估用的 LLM 模型")
    ...
    list_candidates_parser.add_argument(
        "--status", default="", help="按状态过滤，如 pending/approved/rejected/published"
    )
...
def main() -> None:
    ...
    if args.command == "auto-label-failures":
        auto_label_failures_online(
            limit=args.limit,
            sync_failures=not args.skip_sync_failures,
            generate_model=args.generate_model or None,
            eval_model=args.eval_model or None,
        )
        return
