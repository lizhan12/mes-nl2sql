...
class GraphState(TypedDict):
    ...
    retrieval_details: list[dict]  # 并行检索的每条查询词独立结果

    # ---- 节点3 输出 ----
    expanded_tables: str  # BFS 扩展后的表名列表（逗号分隔）
    join_hints: str  # JOIN 提示文本（BFS 推断的参考性 JOIN）
    hard_constraints: str  # 硬约束文本（主表/必含表/必选 JOIN，物理分离自 join_hints）
    ...
    streaming: bool  # 是否启用 LLM token 级流式输出（由 API 参数传入）
