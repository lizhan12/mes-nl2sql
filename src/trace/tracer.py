...
def _extract_token_usage(response: Any) -> dict[str, Any]:
    """从 LLM 响应中提取 token 用量，兼容多种格式。

    OpenAI 原生：response_metadata["token_usage"]
    vLLM 部署：response_metadata["usage"]
    langchain >= 0.3：usage_metadata (ChatOpenAI 内置)
    """
    try:
        # 路径1：langchain >= 0.3 的 usage_metadata
        ...
        # 路径2：response_metadata (来自原始 API 响应)
        response_metadata = getattr(response, "response_metadata", {}) or {}

        # OpenAI 原生格式：response_metadata["token_usage"]
        ...
        # vLLM 格式：response_metadata["usage"]
...
def _truncate(text: str, max_chars: int | None = None) -> str:
    ...
    if max_chars is None:
        max_chars = settings.trace_max_preview_chars
    if not text:
        return ""
    text = text.strip()
...
def _summarize_state(state: dict[str, Any], keys_to_include: list[str] | None = None) -> dict[str, Any]:
    ...
    for k in keys_to_include:
        ...
        if val is None:
            continue
        if isinstance(val, str) and len(val) > settings.trace_max_preview_chars:
...
def _summarize_output(output: dict[str, Any]) -> dict[str, Any]:
    ...
    for k, v in output.items():
        if v is None:
            continue
        if isinstance(v, str) and len(v) > settings.trace_max_preview_chars:
...
async def trace_llm_call_astream(
    prompt: str,
    *,
    node_name: str = "",
    retry_seq: int = 0,
    ...
    """异步流式调用 LLM，逐 token 发送到 stream_writer，同时记录 trace。
    ...
    Args:
        ...
        retry_seq: 重试序号（0=首次）
        model: LLM 模型名
        stream_writer: LangGraph get_stream_writer() 返回的写入器（可选，为 None 时仅收集不改出）

    Returns:
        完整的 LLM 响应内容字符串（已去除 thinking 标签）
    """
    from src.services.llm import get_llm as _get_llm

    trace_id = _trace_id_ctx.get()
    actual_model = model or settings.llm_model

    if not trace_id or not settings.trace_enabled:
        llm = _get_llm(actual_model, streaming=True)
        full_content = ""
        async for chunk in llm.astream(prompt):
            ...
            if content:
                ...
                if stream_writer:
                    stream_writer({"event": "llm_chunk", "node": node_name, "content": content})
        # 移除 thinking 标签
        cleaned = strip_thinking(full_content)
        return cleaned
    ...
    try:
        ...
        # 注意：流式模式下 token 用量通常无法从 chunk 精确获取，通过估算填充
        span.status = STATUS_SUCCESS
        span.output_data = {"response_length": len(cleaned)}
        span.response_preview = _truncate(cleaned)
        span.end_time = time.time()
        span.duration_ms = int((span.end_time - span.start_time) * 1000)
        _persist_span(span)
    ...
    *,
    ...
    if not trace_id or not settings.trace_enabled:
        ...
        if hasattr(response, "content"):
            # 移除 Qwen3 模型的  标签
            clean = strip_thinking(response.content)
    ...
    try:
        ...
        # 提取 token 用量（兼容 OpenAI 原生 / vLLM / langchain 不同版本的字段名）
        raw_content = response.content if hasattr(response, "content") else str(response)
        # 移除 Qwen3 模型的  标签，确保下游代码不看到思考过程
        content = strip_thinking(raw_content)
        if content != raw_content and hasattr(response, "content"):
