...
def load_graph_from_neo4j() -> dict[str, list[dict]]:
...
def replace_all_graph(graph: dict[str, list[dict]]) -> int:
...
def add_edge(from_table: str, to_table: str, edge: dict) -> None:
    """添加一条 JOIN_REL 边。"""
    driver = _get_driver()
    with driver.session() as session:
        session.run("MERGE (t:Table {name: $name})", {"name": from_table})
        session.run("MERGE (t:Table {name: $name})", {"name": to_table})
        session.run(
            """
...
def delete_edge(from_table: str, to_table: str) -> None:
...
def update_edge(from_table: str, to_table: str, edge: dict) -> bool:
    """更新一条 JOIN_REL 边的属性。
...
def get_edge(from_table: str, to_table: str) -> dict | None:
    """获取单条 JOIN_REL 边的详情。"""
    driver = _get_driver()
    with driver.session() as session:
        result = session.run(
...
def list_edges(from_table: str = "", confidence: str = "", limit: int = 500) -> list[dict]:
    """列表查询 JOIN_REL 边。"""
    driver = _get_driver()
    conditions = []
...
def count_graph() -> tuple[int, int]:
    ...
    driver = _get_driver()
    with driver.session() as session:
        node_count = session.run("MATCH (n:Table) RETURN count(n) AS c").single()["c"]
...
def ensure_vector_indexes() -> None:
    """确保 Neo4j 中存在所需的向量索引（schema + few_shot）。"""
    driver = _get_driver()
    with driver.session() as session:
        try:
            session.run(
...
def batch_set_schema_embeddings(items: list[dict]) -> int:
...
def batch_set_few_shot_embeddings(items: list[dict]) -> int:
...
def clear_few_shot_nodes() -> int:
    """删除所有 FewShot 节点。"""
    driver = _get_driver()
    with driver.session() as session:
        result = session.run("MATCH (f:FewShot) DETACH DELETE f RETURN count(f) AS deleted")
...
def schema_has_embeddings() -> bool:
    """检查 Table 节点是否已有 schema_embedding。"""
    driver = _get_driver()
    with driver.session() as session:
        result = session.run("MATCH (t:Table) WHERE t.schema_embedding IS NOT NULL RETURN count(t) AS c").single()
        return bool(result and result["c"] > 0)


def few_shot_has_embeddings() -> bool:
    """检查 FewShot 节点是否已有数据。"""
    driver = _get_driver()
    with driver.session() as session:
        result = session.run("MATCH (f:FewShot) RETURN count(f) AS c").single()
        return bool(result and result["c"] > 0)
...
def ensure_harness_knowledge_indexes() -> None:
    ...
    with driver.session() as session:
        ...
        session.run("CREATE CONSTRAINT few_shot_id_unique IF NOT EXISTS FOR (f:EvolvedFewShot) REQUIRE f.id IS UNIQUE")
        session.run(
...
def publish_harness_knowledge(
    ...
    with driver.session() as session:
        # 1. 清空旧数据
        session.run("MATCH (r:RuntimeRule) DETACH DELETE r")
        session.run("MATCH (f:EvolvedFewShot) DETACH DELETE f")
        session.run("MATCH (v:KnowledgeVersion) DETACH DELETE v")
        ...
        for rule in rules:
            ...
            session.run(
                ...
                """,
                {
                    ...
                    "tables": json.dumps(rule.get("tables", []), ensure_ascii=False),
                    "joins": json.dumps(rule.get("joins", []), ensure_ascii=False),
...
def load_published_rules() -> list[dict]:
    """从 Neo4j 加载已发布的运行时规则。"""
    driver = _get_driver()
    with driver.session() as session:
        result = session.run(
        ...
        for rec in result:
            ...
            # 还原 JSON 数组
            tables_raw = rec["tables"]
            if isinstance(tables_raw, str):
                try:
                    rule["tables"] = json.loads(tables_raw)
                except (json.JSONDecodeError, TypeError):
                    rule["tables"] = []
            else:
                rule["tables"] = tables_raw or []

            joins_raw = rec["joins"]
            if isinstance(joins_raw, str):
                try:
                    rule["joins"] = json.loads(joins_raw)
                except (json.JSONDecodeError, TypeError):
                    rule["joins"] = []
            else:
                rule["joins"] = joins_raw or []
...
def load_published_few_shot_text() -> str:
    ...
    with driver.session() as session:
        ...
        chunks = [rec["full_text"] for rec in result]
    text = "\n---\n".join(chunks) if chunks else ""
    logger.info("从 Neo4j 加载进化 few-shot: %d 条", len(chunks))
    return text


def get_harness_knowledge_version() -> str:
    """获取当前 Harness 知识版本号。"""
    driver = _get_driver()
    with driver.session() as session:
        result = session.run("MATCH (v:KnowledgeVersion {id: 'current'}) RETURN v.version AS version")
        rec = result.single()
...
def init_neo4j_graph() -> None:
    ...
    edge_count = replace_all_graph(graph)
