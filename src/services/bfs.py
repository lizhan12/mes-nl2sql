...
图数据加载策略：
  ...
  本地 JSON 文件仅用于初始化（neo4j_graph.init_neo4j_graph），不再作为查询降级源
"""
...
async def bfs_expand(
    ...
    # 种子表（required_tables）明确指定的表不应被排除，从排除列表中移除其前缀
    if exclude_prefixes and seed_tables:
        seed_prefixes = {t[: t.index("_", t.index("_") + 1) + 1] for t in seed_tables if "_" in t}
        exclude_prefixes = [p for p in exclude_prefixes if p not in seed_prefixes]

    intent_domain_set: set[str] = set(intent_domains) if intent_domains else set()

    # ── 代价模式 ──
    if max_cost is not None:
        return await _bfs_expand_cost(seed_tables, max_cost, max_tables, exclude_prefixes, intent_domain_set)
    ...
    while queue and len(visited) < max_tables:
        ...
        neighbors = (await _get_graph()).get(current, [])
...
async def _bfs_expand_cost(
    ...
    while queue and len(visited) < max_tables:
        ...
        for edge in (await _get_graph()).get(current, []):
            neighbor = edge["to"]
            if neighbor in visited:
                continue
            if len(visited) >= max_tables:
            ...
            if (
                confidence == "medium"
                and intent_domains
                and neighbor_domain not in intent_domains
                and neighbor_domain != "master"
            ):
                continue
...
# ── JOIN 提示文本 ────────────────────────────────────────────────
async def build_path_join_hints(path: list[str]) -> list[dict]:
...
async def build_chain_join_hints(path: list[str]) -> str:
    """将路径转换为带编号的 JOIN 链提示文本，强调 JOIN 顺序。

    与 build_join_hints 的区别：这里会标注步骤序号，让 LLM 明确知道 JOIN 的先后顺序。
    """
    hints = await build_path_join_hints(path)
    if not hints:
        return ""
